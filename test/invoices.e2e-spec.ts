...
jest.setTimeout(60000); // Increased timeout for potentially slow remote DB
...
describe('InvoicesController (e2e)', () => {
  ...
  it('/invoices (POST) - Should create a new draft invoice', async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .expect((res) => {
        ...
        if (res.status === 400 && res.body.message.includes('Product')) return;
        if (res.status === 404 && res.body.message.includes('Product')) return;
  ...
  it('/invoices/issued/:id (GET) - Should get invoice details', async () => {
    ...
    expect(response.body.issuerBusinessId).toBe(businessId);
  });
  ...
  it('/invoices/issued/:id (PATCH) - Should update draft invoice', async () => {
    ...
    expect(response.body.description).toBe('Updated draft during E2E');
  });
