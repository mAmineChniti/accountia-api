...
export class ExpensesService {
  ...
  async getSummary(
    ...
    const totalAmount = categoryAgg.reduce(
      (sum: number, item: AggResult) => sum + item.total,
      0
    );

    return {
      totalAmount,
      byCategory: categoryAgg.map((item: AggResult) => ({
        category: item._id,
        total: item.total,
        count: item.count,
      })),
      byStatus: statusAgg.map((item: AggResult) => ({
        status: item._id,
        total: item.total,
        count: item.count,
      })),
