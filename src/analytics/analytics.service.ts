...
export class AnalyticsService {
  ...
  private async getRevenueTimeline(
    ...
    const result: TimelineResult[] = await invoiceModel.aggregate(pipeline);
  ...
  private async getTopClients(
    ...
    const result: TopClientResult[] = await invoiceModel.aggregate([
      {
        $match: {
          issuerBusinessId: bId,
          status: { $ne: InvoiceStatus.DRAFT },
  ...
  private async getExpenseSummary(
    ...
    try {
      ...
      const bId = new Types.ObjectId(businessId);
      const result: ExpenseAggResult[] = await expenseModel.aggregate([
        {
          $match: {
            businessId: bId,
