...
export class InvoiceCronService {
  ...
  async handleRecurringInvoices() {
    ...
    const dueInvoices = await this.recurringModel
      .find({
    ...
    this.logger.log(
      `Found ${dueInvoices.length} recurring invoices due for generation.`
    );
    ...
    for (const invoiceConfig of dueInvoices) {
      try {
        ...
        successCount++;
        this.logger.log(
          `Generated invoice for schedule ${invoiceConfig._id.toString()} (Client: ${invoiceConfig.clientName})`
        );
      } catch (error) {
        failCount++;
        this.logger.error(
          `Failed to generate invoice for schedule ${invoiceConfig._id.toString()}:`,
          error
        );
      }
    }
    ...
    this.logger.log(
      `Finished processing recurring invoices. Success: ${successCount}, Failed: ${failCount}`
    );
  }
}
