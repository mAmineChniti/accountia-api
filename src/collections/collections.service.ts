...
export class CollectionsService {
  ...
  private groqClientPromise?: Promise<OpenAI>;
  ...
  constructor(
    ...
    this.groqTimeoutMs = this.resolveTimeoutMs(process.env.GROQ_TIMEOUT_MS);
  }
  ...
  /** Lazy-initialise Groq client via OpenAI SDK */
  private async getGroqClient(): Promise<OpenAI> {
    if (!this.groqApiKey) {
      throw new ServiceUnavailableException('GROQ_API_KEY not configured');
    }
    this.groqClientPromise ??= Promise.resolve(
      new OpenAI({
        apiKey: this.groqApiKey,
        baseURL: 'https://api.groq.com/openai/v1',
      })
    );
    return this.groqClientPromise;
  }
   ...
   * Uses Groq to generate a personalised French reminder email.
   */
  async generateReminder(
    ...
    if (!this.groqApiKey) {
      throw new ServiceUnavailableException(
        'GROQ_API_KEY is not configured. Cannot generate reminders.'
      );
    }
...
Génère un email de relance complet avec objet et corps du message.`;
    ...
    try {
      ...
      if (typeof content === 'string') {
        ...
        throw new TypeError('Empty response from Groq');
      }
    } catch (error) {
